...
Inspect the existing interface and supplied references first. For a scoped edit, work inside the established identity and behavior. For a new direction, open one real reference of comparable craft and keep it beside the work; match its finish, not its look. Match the brief's ambition: when it asks for spectacle, the signature moment is the spectacle.
...
Choose one signature motion and choreograph it as a sequence: the invitation, the response, and the arrival, each with a stage of its own. Build it early and finish it before decorating anything. Smaller motions inherit its easing and tempo. Make it understandable on pointer, touch, and keyboard, with a coherent reduced-motion state. Keep controls beside the content they affect.
...
Inspect the complete page at desktop, phone, and an intermediate width, at normal zoom; for scoped edits, the affected components and states. Exercise the main journey and relevant failure states. Read every visible line, including text inside scaled SVGs and canvases at its displayed size. For each line, name the element it belongs to and any other line that already says it; delete companions and repeats, and count the first screen against its budget of five. Then fix small type, weak contrast, awkward wrapping, and competing emphasis. Check touch-target sizes.
...
Fix the largest visible or behavioral defect, then repeat the affected check. Finish with working interactions and a clean visual review, and report anything that could not be verified.
