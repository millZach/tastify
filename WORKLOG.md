...
## Red Rising and Pip comparisons in the README — 2026-09-12

Design 12 (Pip pixel art editor, `design-12a` / `design-12as`): the control was told "Do not use the tastify skill" and never read the file; the skill run started with `$tastify` and read v7. Builds were current with their sources.

Captured designs 11 and 12 from their production builds (served statically on ports 4321–4324) with `examples/capture.mjs`, now filterable with `ONLY=` and `WAIT=`, and launched with SwiftShader because the Red Rising skill page crashed the headless GPU process. 5 s settle. `composite.py` gains an `EXTRA` list: pair images for Red Rising and Pip, grid unchanged. README adds a "Later pairs, skill v7" section with both briefs, desktop and phone pairs, and a table row for design 12. First-screen read of the captures: Red Rising builds share an art direction and differ mainly in the baseline's eyebrow, second tagline, scroll cue, and tag strip; the Pip baseline opens with an eyebrow, flourish, checkmarks, three handwritten asides, a demo caption, and a tiny-type feature strip, while the skill version keeps a headline, one sentence, one button, and a larger demo, with two small captions left around the editor.
